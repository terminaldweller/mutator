...
static W_Code_Section* convert_W_Code_Section (lua_State* __ls, int index) {
	W_Code_Section* dummy = (W_Code_Section*)lua_touserdata(__ls, index);
	if (dummy == NULL) printf("W_Code_Section:bad user data type.\n");
	return dummy;
}

static W_Code_Section* check_W_Code_Section(lua_State* __ls, int index) {
	W_Code_Section* dummy;
	luaL_checktype(__ls, index, LUA_TUSERDATA);
	dummy = (W_Code_Section*)luaL_checkudata(__ls, index, "W_Code_Section");
	if (dummy == NULL) printf("W_Code_Section:bad user data type.\n");
	return dummy;
}
...
W_Code_Section* push_W_Code_Section(lua_State* __ls) {
	...
	W_Code_Section* dummy = lua_newuserdata(__ls, sizeof(W_Code_Section));
	luaL_getmetatable(__ls, "W_Code_Section");
...
int W_Code_Section_push_args(lua_State* __ls, W_Code_Section* _st) {
	...
	lua_pushlightuserdata(__ls, _st->bodies);
	return 0;
}
...
int new_W_Code_Section(lua_State* __ls) {
	...
	W_Function_Body** bodies = lua_touserdata(__ls,-1);
	W_Code_Section* dummy = push_W_Code_Section(__ls);
	dummy->count = count;
	dummy->bodies = bodies;
	return 1;
}
...
static int getter_W_Code_Section_bodies(lua_State* __ls) {
	W_Code_Section* dummy = check_W_Code_Section(__ls, 1);
	lua_pop(__ls, -1);
	lua_pushlightuserdata(__ls, dummy->bodies);
	return 1;
}
...
static const luaL_Reg W_Code_Section_methods[] = {
	{"new", new_W_Code_Section},
	{"set_count", setter_W_Code_Section_count},
	{"set_bodies", setter_W_Code_Section_bodies},
	{"count", getter_W_Code_Section_count},
	{"bodies", getter_W_Code_Section_bodies},
	{0,0}
};

static const luaL_Reg W_Code_Section_meta[] = {
	{0, 0}
};

int W_Code_Section_register(lua_State* __ls) {
	luaL_openlib(__ls, "W_Code_Section", W_Code_Section_methods, 0);
	luaL_newmetatable(__ls, "W_Code_Section");
	luaL_openlib(__ls, 0, W_Code_Section_meta, 0);
