...
static W_Element_Section* convert_W_Element_Section (lua_State* __ls, int index) {
	W_Element_Section* dummy = (W_Element_Section*)lua_touserdata(__ls, index);
	if (dummy == NULL) printf("W_Element_Section:bad user data type.\n");
	return dummy;
}

static W_Element_Section* check_W_Element_Section(lua_State* __ls, int index) {
	W_Element_Section* dummy;
	luaL_checktype(__ls, index, LUA_TUSERDATA);
	dummy = (W_Element_Section*)luaL_checkudata(__ls, index, "W_Element_Section");
	if (dummy == NULL) printf("W_Element_Section:bad user data type.\n");
	return dummy;
}
...
W_Element_Section* push_W_Element_Section(lua_State* __ls) {
	...
	W_Element_Section* dummy = lua_newuserdata(__ls, sizeof(W_Element_Section));
	luaL_getmetatable(__ls, "W_Element_Section");
...
int new_W_Element_Section(lua_State* __ls) {
	...
	W_Elem_Segment** entries = lua_touserdata(__ls,-1);
	W_Element_Section* dummy = push_W_Element_Section(__ls);
	dummy->count = count;
