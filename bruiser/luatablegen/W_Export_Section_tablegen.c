...
static W_Export_Section* convert_W_Export_Section (lua_State* __ls, int index) {
	W_Export_Section* dummy = (W_Export_Section*)lua_touserdata(__ls, index);
	if (dummy == NULL) printf("W_Export_Section:bad user data type.\n");
	return dummy;
}

static W_Export_Section* check_W_Export_Section(lua_State* __ls, int index) {
	W_Export_Section* dummy;
	luaL_checktype(__ls, index, LUA_TUSERDATA);
	dummy = (W_Export_Section*)luaL_checkudata(__ls, index, "W_Export_Section");
	if (dummy == NULL) printf("W_Export_Section:bad user data type.\n");
	return dummy;
}
...
W_Export_Section* push_W_Export_Section(lua_State* __ls) {
	...
	W_Export_Section* dummy = lua_newuserdata(__ls, sizeof(W_Export_Section));
	luaL_getmetatable(__ls, "W_Export_Section");
...
int new_W_Export_Section(lua_State* __ls) {
	...
	W_Export_Entry** entries = lua_touserdata(__ls,-1);
	W_Export_Section* dummy = push_W_Export_Section(__ls);
	dummy->count = count;
...
static const luaL_Reg W_Export_Section_methods[] = {
	{"new", new_W_Export_Section},
	{"set_count", setter_W_Export_Section_count},
	{"set_entries", setter_W_Export_Section_entries},
	{"count", getter_W_Export_Section_count},
	{"entries", getter_W_Export_Section_entries},
	{0,0}
};

static const luaL_Reg W_Export_Section_meta[] = {
	{0, 0}
};

int W_Export_Section_register(lua_State* __ls) {
	luaL_openlib(__ls, "W_Export_Section", W_Export_Section_methods, 0);
	luaL_newmetatable(__ls, "W_Export_Section");
	luaL_openlib(__ls, 0, W_Export_Section_meta, 0);
